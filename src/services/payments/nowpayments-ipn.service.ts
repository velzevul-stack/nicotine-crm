...
import { CryptoPaymentEntity, ReferralEarningEntity, UserEntity } from '@/lib/db/entities';
import {
  verifyIpnSignature,
...
export async function processNowpaymentsIpn(body: IpnPayload, signatureHeader: string | null) {
  if (!signatureHeader) {
    console.error('[IPN] Missing signature header');
    throw new ValidationError('Missing signature', undefined, { code: 'MISSING_SIGNATURE', status: 400 });
  }

  if (!verifyIpnSignature(body as unknown as Record<string, unknown>, signatureHeader)) {
  ...
  payment.status = body.payment_status as (typeof payment)['status'];
    ...
    if (user.referrerId && now < REFERRAL_PROGRAM_END) {
      ...
      if (referrer) {
        ...
        if (
          ...
          new Date(referrer.subscriptionEndsAt) > referrerNow
        ) {
  ...
  if (botToken) {
    try {
      ...
      if (referrerTelegramId && referrerEndsAt) {
        await bot.telegram.sendMessage(
          parseInt(referrerTelegramId, 10),
          `🎉 Поздравляем!\n\n` +
