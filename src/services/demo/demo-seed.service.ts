...
const DISABLED_MESSAGE =
  'This endpoint is disabled. Use npm run db:seed instead.';

/** HTTP-сид магазина отключён: используйте `npm run db:seed`. */
export function demoHttpSeedDisabledError(): ForbiddenError {
  return new ForbiddenError(DISABLED_MESSAGE);
}

/*
 * Ниже — прежняя реализация POST (для справки, не исполняется).
 * Требовалась сессия, пустой список категорий у shopId, репозитории Category/Brand/ProductFormat/Flavor/StockItem.
  ...
  const pfMap = { pf1, pf2, pf3, pf4, pf5, pf6, pf7, pf8, pf9, pf10 };
  ...
  for (let i = 0; i < flavorNames.length; i++) {
    ...
    const format = pfMap[pfKey as keyof typeof pfMap];
