...
bot.on('text', async (ctx) => {
  ...
  // Старый код генерации поста (оставлен для совместимости, но не используется)
  if (false && text === '📝 Пост (старый)') {
    // Генерация поста для копирования
    if (user.role !== 'seller') {
      await ctx.reply('❌ Эта команда доступна только продавцам.');
      return;
    }
    ...
    try {
      ...
      // Получаем данные для генерации поста
      const [categories, brands, formats, flavors, stocks, shop, postFormats] = await ds.transaction(async (em) => {
        ...
        return Promise.all([
          ...
          formatRepo.find({
            where: { shopId: userShop.shopId, isActive: true },
          }),
          flavorRepo.find({
            where: { shopId: userShop.shopId, isActive: true },
          }),
      ...
      const reservationItems = reservationIds.length > 0
        ? await ds.getRepository(SaleItemEntity).find({ where: { saleId: In(reservationIds) } })
        : [];
      ...
      // Получаем активный формат поста или используем дефолтный
      let template: string;
      let formatConfig: FormatConfig = {};
      
      const activePostFormat = postFormats.length > 0 ? postFormats[0] : null;
      if (activePostFormat) {
        template = activePostFormat.template;
        formatConfig = (activePostFormat.config as FormatConfig) || {};
      } else {
...
{content}`;
      ...
      // Фильтруем форматы - показываем только те, у которых есть доступные остатки
      const filteredFormats = formats.filter((f) => {
        const formatFlavors = flavors.filter((fl) => fl.productFormatId === f.id);
        return formatFlavors.some((flavor) => {
          const stock = stockMap.get(flavor.id);
          const qty = stock?.quantity ?? 0;
          const reserved = reservedQtyByFlavorId.get(flavor.id) ?? 0;
      ...
      if (!postText || postText.trim().length === 0) {
        ...
        await ctx.reply('❌ Не удалось сгенерировать пост. Возможно, у вас нет товаров в наличии.');
        return;
      }
      ...
      try {
        ...
        // Игнорируем ошибку удаления сообщения (может быть старше 48 часов)
      }

      // Отправляем текст поста
      await ctx.reply(`📝 Ваш пост для копирования:\n\n\`\`\`\n${postText}\n\`\`\``, {
        parse_mode: 'Markdown',
      });
    } catch (error) {
