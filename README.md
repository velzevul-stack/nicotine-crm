...
**Mini App и сид:** демо-данные по умолчанию привязываются к пользователю `dev-user-1` (вход через `/login` в dev). Чтобы тот же склад был в **мини-приложении** под вашим Telegram:

1. Один раз зайдите в бота или откройте Mini App (создастся пользователь с вашим `telegramId`).
2. Запустите `npm run db:seed` — сид найдёт аккаунт **@wendigo2347** (или пользователя с `SEED_OWNER_TELEGRAM_ID` в `.env`) и наполнит **его** магазин.

После `npm run db:clear` заново войдите в бот/миниап (при необходимости), затем снова `db:seed`.
